...
function billCalculator(billAmount, numberOfPeople){
    return (parseFloat((billAmount / numberOfPeople)) + parseFloat(tipAmountPerPerson)).toFixed(2);
}

function tipCalculator(tipPercent, billAmount, numberOfPeople){
    tipAmountPerPerson = (((tipPercent / 100) * billAmount) / numberOfPeople).toFixed(2);
...
reset.addEventListener("click", function(){
    ...
    document.querySelector("small").reset();
